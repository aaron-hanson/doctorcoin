...
#if defined(NDEBUG)
# error "Doctorcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Doctorcoin: starting difficulty is 1 / 2^12
...
{
    // Doctorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // BIP 42
    int halvings = nHeight / 1235124;
    if (halvings >= 64)
        return nFees;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= halvings; // Doctorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

//static const int64 nTargetTimespan = 1.4 * 24 * 60 * 60; // Doctorcoin: 1.4 days
static const int64 nTargetTimespan = 5 * 60; // Doctorcoin: 1.4 days
static const int64 nTargetSpacing = 60; // Doctorcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Doctorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doctorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8ab98409b6e1715e09cb8119b284b38203194edc6d86b834c894e772bd796ed4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Chicago Tribune 28/Aug/2015 Stalemate Building On Itself";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xba3b8d512fbe84ac9e31aed2ea6bdef646625946f538e25c98c70841e104e61a"));
...
unsigned char pchMessageStart[4] = { 0xd1, 0x5e, 0xa5, 0xe5 }; // Doctorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoctorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoctorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoctorcoinMiner(CWallet *pwallet)
{
    printf("DoctorcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DoctorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoctorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoctorcoinMiner, pwallet));
}
